...
func loadSystemLanguage() string {
	...
	if langStr == "" {
		klog.V(3).Infof("Couldn't find the LC_ALL, LC_MESSAGES or LANG environment variables, defaulting to zh_CN")
		return "default"
	}
	pieces := strings.Split(langStr, ".")
	if len(pieces) != 2 {
		klog.V(3).Infof("Unexpected system language (%s), defaulting to zh_CN", langStr)
		return "default"
	}
	return pieces[0]
}
...
func LoadTranslations(root string, getLanguageFn func() string) error {
	...
	translationFiles := []string{
		fmt.Sprintf("%s/%s/LC_MESSAGES/karmada.po", root, langStr),
		fmt.Sprintf("%s/%s/LC_MESSAGES/karmada.mo", root, langStr),
	}
	...
	gettext.BindTextdomain("karmada", root+".zip", buf.Bytes())
	gettext.Textdomain("karmada")
